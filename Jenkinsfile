...
stages {
    ...
    stage('MVN PACKAGE'){
      ...
      stage("Nexus deploy"){
                           steps {
                              sh 'mvn deploy:deploy-file -DgroupId=com.esprit.examen -DartifactId=tpAchatProject -Dversion=6.1 -DgeneratePom=true -Dpackaging=jar -DrepositoryId=deploymentRepo -Durl=http://192.168.1.195:8081/repository/maven-releases -Dfile=target/docker-spring-boot.jar'
                                   }
                                    }
    stage('MVN SONARQUBE'){
               steps{
